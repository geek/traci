...
function onRequest (request, h) {
  ...
  let span;
  if (request.spans && request.spans.parentRequest) {
    span = request.server.tracer.startSpan('hapi_request', { childOf: request.spans.parentRequest, startTime: now });
  } else {
    span = request.server.tracer.startSpan('hapi_request', { startTime: now });
  }
  ...
  if (!request.spans) {
    request.server.decorate('request', 'spans', {});
    request.server.decorate('request', 'span', requestSpan);
  }

  request.spans[`request_${request.info.id}`] = span;
  request.spans.parentRequest = span;
...
function finishSpans (spans) {
  ...
  for (const key of Object.keys(spans)) {
    // Check that finish hasn't been called yet (jaeger span and mock span)
    if (!spans[key]._duration && !spans[key]._finishMs) {
...
function requestSpan () {
  return this.spans[`request_${this.info.id}`];
}
