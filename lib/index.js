...
function onRequest (request, h) {
  ...
  request.plugins.traci.spans[`request_${request.info.id}`] = span;
  return h.continue;
}
...
function onPreAuth (request, h) {
  ...
  span.log({ event, ...request.pickSpanLog(event) }, now);
  request.plugins.traci.spans[`auth_${request.info.id}`] = span;
...
function onPostAuth (request, h) {
  ...
  const span = request.plugins.traci.spans[`auth_${request.info.id}`];
...
function onPreHandler (request, h) {
  ...
  span.log({ event, ...request.pickSpanLog(event) }, now);
  request.plugins.traci.spans[`handler_${request.info.id}`] = span;
...
function onPreResponse (request, h) {
  ...
  if (request.plugins.traci && request.plugins.traci.spans[`handler_${request.info.id}`]) {
    request.plugins.traci.spans[`handler_${request.info.id}`].finish(now);
  }
  ...
  request.plugins.traci.spans[`response_${request.info.id}`] = span;
...
function requestSpan (spanKind = 'request') {
  if (this.plugins.traci) {
    return this.plugins.traci.spans[`${spanKind}_${this.info.id}`];
  }
}
